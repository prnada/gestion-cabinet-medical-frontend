...
const ListInfs = () => {
  ...
  const [showModel,setShowModel] = useState(false)
    ...
    useEffect(()=> {
        axios.get("https://localhost:7285/api/Infermiers")
        .then((response)=>{
            setInfermiers((existingData)=>{
    ...
    function showDeleteConfirmation(id){
      ...
      function deleteConfirmHandler(){
        axios.delete(`https://localhost:7285/api/Infermiers/${itemToDelete}`)
        .then((response)=>{
          setInfermiers((existingData)=>{
...
<div className="main-content">
         ...
         <div className="table-title">
          <div className="row">
            <div className="col-sm-6 p-0 flex justify-content-lg-start justify-content-center">
              <h2 className="ml-lg-2">Gérer les infermiers</h2>
            </div>
            <DeleteConfirmation 
        showModel={showModel}
        title="Delete Comfirmation"
        body="Êtes-vous sûr(e) de vouloir supprimer cet infirmier(e) ?"
        ...
        deleteConfirmHandler={deleteConfirmHandler} ></DeleteConfirmation>
            <div className="col-sm-6 p-0 flex justify-content-lg-end justify-content-center">
              <a href="#addEmployeeModal" className="btn btn-success" data-toggle="modal">
                ...
                <Link to="/new-infermier" >Ajouter un infermier</Link>
              </a>
        ...
        <table className="table table-striped table-hover">
          ...
          <tbody>
          {
                 Infermiers.map((infermier)=> {
                    return <tr key={infermier.id}>
                      <td>{infermier.nomInf}</td>
                      <td>{infermier.prenomInf}</td>
                      <td>{infermier.nomtele}</td>
                      
                      <td>{infermier.email}</td>
                      <td>{infermier.nomM +' '+ infermier.prenomM}</td>
               
                  <td>
                  <Button variant='success' type='button' onClick={()=> {
                      navigate(`/update-infermier/${infermier.id}`)
                   }}>
                      <i className="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i>
                   </Button>
                 
              
                <Button variant='danger' type='button' onClick={()=>{ showDeleteConfirmation(infermier.id);}}>
